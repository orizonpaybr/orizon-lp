  ...
  const onSubmit = async (data: OuvidoriaFormValues) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        toast.success(
          `Manifestação enviada com sucesso! Protocolo: ${result.protocol}`,
        )
        reset()
        setSelectedFiles([])
      } else {
        toast.error(
          result.error || 'Erro ao enviar manifestação. Tente novamente.',
        )
      }
    } catch (error) {
      ...
      <Section className="bg-background py-16 md:py-24">
        <Container>
          <div className="max-w-2xl mx-auto">
            <div className="bg-surface border border-border rounded-2xl shadow-xl p-8 md:p-12">
              ...
              <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
                      ...
                      render={({ field }) => (
                        <IMaskInput
                          ...
                          className={`
                            ...
                            ${errors.cpf ? 'border-red-500' : 'border-border'}
                          `}
                          {...field}
                        />
                      )}
                  ...
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1">
                      <p className="text-sm text-muted">
                        Para que possamos analisar sua transferência com mais
                        agilidade, pedimos que anexe o comprovante original da
                        transação.
                      </p>
                      <p className="text-xs text-muted-foreground mt-1">
                        Formatos aceitos: JPG, PNG, GIF, PDF, TXT. Máximo 5MB
                        por arquivo.
                      </p>
                    </div>
                  </div>
                      ...
                      {selectedFiles.map((file, index) => (
                        ...
                        >
                          <div className="flex items-center gap-3">
                            <HiDocumentText className="text-muted" size={20} />
                            <div>
                ...
                <p className="text-sm text-muted">(*) Campos obrigatórios</p>
